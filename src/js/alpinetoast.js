...
function maybeParseClasses(maybeString) {
  if (typeof maybeString === 'string' && maybeString.length > 0) {
    return maybeString.split(' ').filter(function (x) {
      return x.length > 0
    })
  }
  return undefined
...
var TailWindCSSConfig = {
  ...
  progressBarClasses:
    'h-1 bg-gray-100 bg-opacity-50 transition-all absolute bottom-0 left-0 right-0',
}
...
var AlpineToast = /** @class */ (function () {
  function AlpineToast(config) {
    ...
    this.progressBarClasses = maybeParseClasses(config.progressBarClasses) || []
  ...
  AlpineToast.prototype.addProgressBar = function (elem) {
    ...
    var classes = ''.split(' ').filter(function (cls) {
      return cls.length > 0
    })
  ...
  AlpineToast.prototype.makeToast = function (elem, args) {
    ...
    var update_timer = setInterval(function () {
      ...
      if (duration <= 0) {
        ...
        // Toggle the classes
        if (onHideClasses !== []) {
          onHideClasses.forEach(function (cls) {
    ...
    setTimeout(function () {
      if (onShowClasses !== []) {
        onShowClasses.forEach(function (cls) {
          return elem.classList.toggle(cls)
        })
      }
    }, 500)
